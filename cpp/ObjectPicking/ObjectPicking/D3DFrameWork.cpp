...
D3DFrameWork* D3DFrameWork::m_pD3DFrameWork = 0;
IDirect3DDevice9* D3DFrameWork::m_pD3DDevice = 0;

//callback function to define message handling
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	// Don't start processing messages until the application has been created.
	if (D3DFrameWork::GetD3DFrameWork())
		return D3DFrameWork::GetD3DFrameWork()->MsgProc(msg, wParam, lParam);
	else
		return DefWindowProc(hwnd, msg, wParam, lParam);
}

void D3DFrameWork::InitBase(HINSTANCE hInstance, string winCaption, D3DDEVTYPE devType, DWORD requestedVP)
{
...
void D3DFrameWork::InitDirect3D()
{
	...
	m_pD3D->GetAdapterIdentifier(0, 0, &ident);
	...
	HR(m_pD3D->CreateDevice(
	    ...
	    &m_pD3DDevice));		//out created device
}
	...
	case WM_SIZE:
		if (m_pD3DDevice)
		{
			...
			else if (wParam == SIZE_MAXIMIZED)
			{
				...
				minOrMaxed = true;
				OnLostDevice();
				HR(m_pD3DDevice->Reset(&m_D3DPrams));
				OnResetDevice();
			}
...
void D3DFrameWork::TouchFullScreenMode(bool enable)
{
	int width  = GetSystemMetrics(SM_CXSCREEN);
	int height = GetSystemMetrics(SM_CYSCREEN);
...
bool D3DFrameWork::IsDeviceLost()
{
