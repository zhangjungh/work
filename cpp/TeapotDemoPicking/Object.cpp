...
void D3DObject::Update(float dt, const D3DXVECTOR3& v)
{
	if (m_bHitTest)
	{
		...
		D3DXMATRIX mWorldView = m_mWorld * m_pCamera->view();
		D3DXMATRIX m;
...
void D3DObject::Render(void)
{
	...
	HR(m_pEffect->GetLightFx()->SetValue(m_pEffect->GetCameraPos(), &m_pCamera->pos(), sizeof(D3DXVECTOR3)));
	HR(m_pEffect->GetLightFx()->SetMatrix(m_pEffect->GetWVPMatrix(), &(m_mWorld * m_pCamera->viewProj())));
	...
	for (UINT i=0; i<numPasses; ++i)
	{
		...
		for(UINT j = 0; j < m_vMats.size(); ++j)
		{
			...
			{
				//HR(m_pEffect->GetLightFx()->SetTexture(m_pEffect->GetTexture(), mWhiteTex));
			}
	...
	if (m_bIsHitted)
	{
		...
		HR(m_pEffect->GetOutlineFx()->SetMatrix(m_pEffect->GetOLWVMatrix(), &(m_mWorld*m_pCamera->view())));
		HR(m_pEffect->GetOutlineFx()->SetMatrix(m_pEffect->GetOLPMatrix(), &(m_pCamera->proj())));
