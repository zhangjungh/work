# -*- coding: utf-8 -*-
# for AES256 Cipher-Block Chaining (CBC) and PKCS5 padding
...
DATAPAD = lambda s: s + (BLOCK_SIZE - len(s) % BLOCK_SIZE) * chr(BLOCK_SIZE - len(s) % BLOCK_SIZE)
DATAUNPAD = lambda s : s[0:-ord(s[-1])]

class BLOCKSTATE():
	def __init__(self):
		self.ek = [0 for i in range( 4*(MAXNR+1) )]
		self.dk = [0 for i in range( 4*(MAXNR+1) )]
		self.rounds = 0
		
class ALObject():
	def __init__(self):
		self.IV = [0 for i in range(BLOCK_SIZE)]
		self.st = BLOCKSTATE()
...
def GETU32(p, c):
	return struct.unpack('>I', ''.join( p[c:c+4] ))[0]

def PUTU32(ct, c, st):
	ct[c:c+4] = [ord(i) for i in struct.pack('>I', st)]

''' both works, process endian issue
def GETU32(p, c):
	return (ord(p[c+0]) << 24) ^ (ord(p[c+1]) << 16) ^ (ord(p[c+2]) << 8) ^ ord(p[c+3])
	...
	if keyBits == 128:
		while True:
			...
			rk[c+4] = rk[c] ^ \
				...
				rcon[i]
			rk[c+5] = rk[c+1] ^ rk[c+4]
			rk[c+6] = rk[c+2] ^ rk[c+5]
			rk[c+7] = rk[c+3] ^ rk[c+6]
			i += 1
			if i == 10:
				return 10
			c += 4
	...
	if keyBits == 192:
		while True:
			...
			rk[c+6] = rk[c+0] ^ \
				...
				(Te4[(temp >> 24)       ] & 0x000000ff) ^ \
				rcon[i]
			rk[c+7] = rk[c+1] ^ rk[c+6]
			rk[c+8] = rk[c+2] ^ rk[c+7]
			rk[c+9] = rk[c+3] ^ rk[c+8]
			i += 1
			if i == 8:
				return 12
			rk[c+10] = rk[c+4] ^ rk[c+9]
			rk[c+11] = rk[c+5] ^ rk[c+10]
			c += 6
	...
	if keyBits == 256:
		while True:
			...
			rk[c+8] = rk[c+0] ^ \
				...
				(Te4[(temp >> 24)       ] & 0x000000ff) ^ \
				rcon[i]
			rk[c+9] = rk[c+1] ^ rk[c+8]
			rk[c+10] = rk[c+2] ^ rk[c+9]
			rk[c+11] = rk[c+3] ^ rk[c+10]
			i += 1
			if i == 7:
				return 14
			temp = rk[c+11]
			...
			rk[c+13] = rk[c+5] ^ rk[c+12]
			rk[c+14] = rk[c+6] ^ rk[c+13]
			rk[c+15] = rk[c+7] ^ rk[c+14]
...
def rijndaelKeySetupDec(rk, cipherKey, keyBits):
	...
	for i in range(1, Nr):
		c += 4
		rk[c+0] = \
			Td0[Te4[(rk[c+0] >> 24)       ] & 0xff] ^ \
			Td1[Te4[(rk[c+0] >> 16) & 0xff] & 0xff] ^ \
			Td2[Te4[(rk[c+0] >>  8) & 0xff] & 0xff] ^ \
			Td3[Te4[(rk[c+0]      ) & 0xff] & 0xff]
		rk[c+1] = \
			Td0[Te4[(rk[c+1] >> 24)       ] & 0xff] ^ \
			Td1[Te4[(rk[c+1] >> 16) & 0xff] & 0xff] ^ \
			Td2[Te4[(rk[c+1] >>  8) & 0xff] & 0xff] ^ \
			Td3[Te4[(rk[c+1]      ) & 0xff] & 0xff]
		rk[c+2] = \
			Td0[Te4[(rk[c+2] >> 24)       ] & 0xff] ^ \
			Td1[Te4[(rk[c+2] >> 16) & 0xff] & 0xff] ^ \
			Td2[Te4[(rk[c+2] >>  8) & 0xff] & 0xff] ^ \
			Td3[Te4[(rk[c+2]      ) & 0xff] & 0xff]
		rk[c+3] = \
			Td0[Te4[(rk[c+3] >> 24)       ] & 0xff] ^ \
			Td1[Te4[(rk[c+3] >> 16) & 0xff] & 0xff] ^ \
			Td2[Te4[(rk[c+3] >>  8) & 0xff] & 0xff] ^ \
			Td3[Te4[(rk[c+3]      ) & 0xff] & 0xff]
...
def block_init(state, key, keylen):
...
def block_encrypt(state, bin, bout):
	rijndaelEncrypt(state.ek, state.rounds, bin, bout)

def block_decrypt(state, bin, bout):
	rijndaelDecrypt(state.dk, state.rounds, bin, bout)

class AESCBC:	
	def __init__(self, key, IV):
		IVlen = len(IV)
		if IVlen != BLOCK_SIZE:
			raise ValueError("IV must be %i bytes long" % BLOCK_SIZE)
		keylen = len(key)
		if KEY_SIZE != 0 and keylen != KEY_SIZE:
			raise ValueError("Key must be %i bytes long, not %i" % KEY_SIZE, keylen)
		if KEY_SIZE == 0 and keylen == 0:
			raise ValueError("Key cannot be the null string")
		self.al = ALObject()
		block_init(self.al.st, key, keylen)
		self.al.IV[:IVlen] = IV
		
	def decrypt(self, cptext):
		cplen = len(cptext)
		if cplen == 0:
			return ''
		buffer = [0 for i in range(cplen)]
		temp = [0 for i in range(BLOCK_SIZE)]
		for i in range(0, cplen, BLOCK_SIZE):
			block_decrypt(self.al.st, cptext[i:i+BLOCK_SIZE], temp);
			for j in range(BLOCK_SIZE):
				buffer[i+j] = (temp[j] ^ ord(self.al.IV[j]))
				self.al.IV[j] = cptext[i+j]
		return DATAUNPAD( ''.join([chr(i) for i in buffer]) )
		

def AES_Decrypt(infile, outfile, iv, salt, iteration, password):
	sb = salt.encode('utf8')
	wb = iv.encode('utf8')
	ivb = ''
	if len(wb) >= BLOCK_SIZE:
		ivb = wb[:BLOCK_SIZE]
	else:
		ivb = wb + ''.join([ chr(int(-100-i) & 0xff) for i in range(len(wb), BLOCK_SIZE) ])
	pbk = PBKDF2(password, sb, iteration)
	cbc = AESCBC(pbk.read(32), ivb) #AES256
	with open(infile, 'rb') as f:
		with open(outfile, 'wb') as w:
			w.write(cbc.decrypt(f.read()))